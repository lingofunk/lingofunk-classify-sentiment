...
def place_check(x, y):
    center = (-81, 41)
    rad = 12.0
    return (x - center[0]) ** 2 + (y - center[1]) ** 2 < rad ** 2
...
if __name__ == '__main__':
    ...
    while row_str != '':
        ...
        if json_str['city'] == CITY:
            ...
            business.update({json_str['business_id']: {'type': 'Feature',
                                                       'geometry': {'type': 'Point',
                                                                    'coordinates': [json_str['longitude'],
                                                                                    json_str['latitude']]},
                                                       'properties': dict().fromkeys([i for i in json_str.keys() if i not in ['latitude', 'longitude']])}})
    ...
    while row_str != '':
        ...
        if json_str['business_id'] in business_ids:
            #business[json_str['business_id']]['properties']['reviews'].append(json_str)
            if len(business_reviews[json_str['business_id']]) < REVIEW_PER_BUSINESS:
                business_reviews[json_str['business_id']].append(json_str)
                r_count += 1
            else:
                business_reviews[json_str['business_id']] = sorted(business_reviews[json_str['business_id']],
                                                                   key=lambda x: -(x['useful'] + x['funny'] + x['cool']))
                if json_str['useful'] + json_str['funny'] + json_str['cool'] > business_reviews[json_str['business_id']][REVIEW_PER_BUSINESS - 1]['useful'] + business_reviews[json_str['business_id']][REVIEW_PER_BUSINESS - 1]['funny'] + business_reviews[json_str['business_id']][REVIEW_PER_BUSINESS - 1]['cool']:
                    business_reviews[json_str['business_id']][REVIEW_PER_BUSINESS - 1] = json_str
        row_str = reviews_input.readline()
    reviews_to_out = list()
    for i in business_reviews.keys():
        business[i]['properties']['reviews'] = sorted(business_reviews[i], key=lambda x: -(x['useful'] + x['funny'] + x['cool']))
        reviews_to_out.extend(business[i]['properties']['reviews'])
    business_to_out = geojson.GeoJSONEncoder().encode({'type': 'FeatureCollection', 'features': list(business.values())})
    pd.DataFrame(reviews_to_out).to_csv(CITY + '_dataset.csv', index=False, encoding='utf-8')
